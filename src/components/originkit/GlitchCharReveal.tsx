...
// This is an imperative, rAF-driven animation engine (character scramble/glitch
// reveal). It intentionally drives React state from effects and reads
// performance.now() for real-time timing — both flagged by the newer,
// React Compiler–oriented lint rules, but correct and necessary here.
 ...
 */
function GlitchCharRevealCore(props: any) {
    ...
    const [displays, setDisplays] = useState<
        Record<string, { char: string; locked: boolean; flickering: boolean }>
    >({})
        ...
        const animateChar = async (
            ...
            const glitchDelay =
                glitchFrames > 0
                    ? Math.max(
                          minDelay,
                          Math.floor((win * 0.8) / glitchFrames)
                      )
                    : minDelay
            if (glitchFrames > 0) {
...
export default function GlitchCharReveal(props: Record<string, unknown>) {
  return <GlitchCharRevealCore {...(__originkitPresetProps as Record<string, unknown>)} {...props} />;
}
